...
from .tusfile import TusFile, TusChunk, FilenameGenerator
...
class TusUpload(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        upload_path = request.build_absolute_uri()
        upload_path.replace("http:","https:")
        
        return TusResponse(
            status=201,
            extra_headers={'Location': '{}{}'.format(upload_path, tus_file.resource_id)})
    ...
    def move(self, tusfile):
        setting = settings.TUS_FILE_NAME_FORMAT

        if setting == 'keep':
            if tusfile.check_existing_file(tusfile.filename):
                return TusResponse(status=409, reason="File with same name already exists")
        elif setting == 'random':
            tusfile.filename = FilenameGenerator(tusfile.filename).create_random_name()
        elif setting == 'random-suffix':
            tusfile.filename = FilenameGenerator(tusfile.filename).create_random_suffix_name()
        elif setting == 'increment':
            tusfile.filename = FilenameGenerator(tusfile.filename).create_incremented_name()
        else:
            return ValueError()

        shutil.move(tusfile.get_path(), os.path.join(settings.TUS_DESTINATION_DIR, tusfile.filename))


    def move_simple(self, tusfile):
        os.makedirs(os.path.join(settings.TUS_DESTINATION_DIR, tusfile.resource_id), exist_ok=True)
        shutil.move(tusfile.get_path(), os.path.join(settings.TUS_DESTINATION_DIR, tusfile.resource_id, tusfile.filename))
